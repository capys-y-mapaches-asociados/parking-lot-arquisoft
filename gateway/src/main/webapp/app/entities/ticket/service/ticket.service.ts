...
type RestOf<T extends ITicket | NewTicket> = Omit<T, 'issuedAt' | 'entryTime' | 'exitTime'> & {
...
export class TicketService {
  ...
  protected convertDateFromClient<T extends ITicket | NewTicket | PartialUpdateTicket>(ticket: T): RestOf<T> {
    return {
      ...ticket,
      issuedAt: ticket.issuedAt?.toJSON() ?? null,
      entryTime: ticket.entryTime?.toJSON() ?? null,
      exitTime: ticket.exitTime?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restTicket: RestTicket): ITicket {
    return {
      ...restTicket,
      issuedAt: restTicket.issuedAt ? dayjs(restTicket.issuedAt) : undefined,
      entryTime: restTicket.entryTime ? dayjs(restTicket.entryTime) : undefined,
      exitTime: restTicket.exitTime ? dayjs(restTicket.exitTime) : undefined,
    };
  }
